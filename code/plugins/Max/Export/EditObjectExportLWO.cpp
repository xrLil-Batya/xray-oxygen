...
int FindLPCSTR(xr_vector<const char*>& vec, LPCSTR key){
...
bool CEditableObject::ExportLWO(LPCSTR fname)
{
	...
	F->begin_save();
		...
		// meshes/layers
		for (auto mesh_it=FirstMesh(); mesh_it!= FirstMesh() + MeshCount(); mesh_it++){
			...
			F->open_chunk(ID_PTAG);
				...
				for (auto sf_it=MESH->m_SurfFaces.begin(); sf_it!=MESH->m_SurfFaces.end(); sf_it++){
					...
					for (auto i_it=lst.begin(); i_it!=lst.end(); i_it++){
						F->w_vx	(*i_it);
