...
{
	string1024 nm;
	xr_strcpy(nm, tex_name);
	tex_name = EFS.AppendFolderToName(nm, sizeof(nm), depth, full_name);
	return tex_name;
}
#ifdef _EDITOR
        ...
        // parse lwo object
        {
            ...
            st_lwSurface* Isf=0;
            {
                ...
                for (Isf=I->surf; Isf; Isf=Isf->next)
				{
					...
					else ELog.Msg(mtError,"CEditableObject: Shader not found on surface '%s'.",Osf->_Name());
#pragma todo("Implemented Resources to Device!")
                    ...
                    for (st_lwTexture* Itx=Isf->color.tex; Itx; Itx=Itx->next){
                        ...
                        if (cidx!=-1){
							...
							Osf->SetTexture(AppendFolderToName(tex_name,1,TRUE).c_str());
                            // get vmap refs
...
//					UI->ProgressStart(Ilr->point.count,"Fill points:");
				    ...
				    std::memset(MESH->m_SmoothGroups,u32(-1),MESH->m_FaceCount);
                    ...
                    for (int i=0; i<Ilr->polygon.count; ++i)
                    {
                        ...
                        for (int pv_i=0; pv_i<3; ++pv_i)
                        {
							...
							using VMapPtIt = VMapVec::iterator;
                            VMapPtVec		vm_lst;
                            ...
                            std::memcpy(m_vm_lst.pts,&*vm_lst.begin(),m_vm_lst.count*sizeof(st_VMapPt));
                        }
                        if (!bResult) break;
