...
//	MSceneMessage::addCallback(MSceneMessage::kMayaExiting,  uninitialize, 0);
	...
	if (status!=MS::kSuccess) return status;
	status = plugin.registerFileTranslator("XRay_Camera_Motion_Export",	"none", CXRayCameraExport::creator, (char *)xraySkinMotionOptionScript, (char *)xrayCameraMotionDefaultOptions);
	if (status!=MS::kSuccess) return status;
