...
#include "../../../editors/xrECoreLite/motion.h"
...
*/
MPxFileTranslator::MFileKind CXRaySkinExport::identifyFile(
...
MStatus CXRaySkinExport::writer(const MFileObject& file, const MString& options, FileAccessMode mode)
{
    ...
    if (options.length() > 0)
    {
        ...
        for (i = 0; i < length; ++i)
        {
            ...
            else if (theOption[0] == MString("locator"))
            {
                if (theOption.length() > 1)
...
MStatus CXRaySkinExport::exportObject(LPCSTR fn, bool b_ogf)
{
    ...
    for (auto& boneIt : m_boneList)
    {
        ...
        OBJECT->Bones().emplace_back(new CBone());
    ...
    {
        Msg("!Invalid mesh: '%s'. Faces<2 or Verts<4", *MESH->Name());
        MGlobal::executeCommand("doEnableNodeItems true all");
        return MStatus::kFailure;
    }
    ...
    xr_strconcat(object_name, fname, ".object");
    xr_strconcat(ogf_name, fname, ".ogf");
    OBJECT->Optimize();
...
MStatus CXRaySkinExport::exportMotion(LPCSTR fn, const MMatrix& locator)
{
    ...
    // cout << "Exporting animation frames from " << frameFirst << " to " << frameLast << "\n";
    {
        ...
        for (auto& bone : m_boneList)
        {
            ...
            BM.envs[ctPositionX]->behavior[0] = 1;
            BM.envs[ctPositionX]->behavior[1] = 1;
            BM.envs[ctPositionY]->behavior[0] = 1;
            BM.envs[ctPositionY]->behavior[1] = 1;
            BM.envs[ctPositionZ]->behavior[0] = 1;
            BM.envs[ctPositionZ]->behavior[1] = 1;
            BM.envs[ctRotationH]->behavior[0] = 1;
            BM.envs[ctRotationH]->behavior[1] = 1;
            BM.envs[ctRotationP]->behavior[0] = 1;
            BM.envs[ctRotationP]->behavior[1] = 1;
            BM.envs[ctRotationB]->behavior[0] = 1;
            BM.envs[ctRotationB]->behavior[1] = 1;
        }
        ...
        for (int i = frameFirst; i <= frameLast; i++)
        {
            ...
            for (auto boneIt = m_boneList.begin(); boneIt != m_boneList.end(); ++boneIt)
            {
                ...
                X->time = displacedTime;
                Y->time = displacedTime;
                Z->time = displacedTime;
                H->time = displacedTime;
                P->time = displacedTime;
                B->time = displacedTime;
...
MStatus CXRaySkinExport::parsePolySet(
    MItMeshPolygon& meshPoly, MObjectArray& rgShaders, MIntArray texMap, int presetBone = -1)
{
    SmdTriangle* tri;
    MPointArray rgpt;
    MIntArray rgint;
    ...
    for (; !meshPoly.isDone(); meshPoly.next())
    {
        ...
        {
            status = MS::kFailure;
            Msg("!polygon have zero area:", meshPoly.index());
            return status;
        }
        ...
        */
        ...
        {
            status = MS::kFailure;
			Msg("!polygon is missing UV information:", meshPoly.index());
            return status;
        }
...
MStatus CXRaySkinExport::CalculateTriangleVertex(
...
namespace TEST
{
	IC MMatrix CalculateFullTransform1(MFnDagNode node)
	{
		MStatus status;
		...
		if (1 == pcnt)
		{
			...
			{
				mat *= CalculateFullTransform1(obj);
			}
		}
		return mat;
	}
...
MStatus CXRaySkinExport::getBoneData(const MMatrix& locator)
{
    ...
    if (status == MS::kSuccess)
    {
        for (auto& itBones : m_boneList)
        {
			MFnDagNode fnJoint = (MFnDagNode)(itBones->path.node(&status));
            ...
            if (itBones->parentId == -1)
            {
                MMatrix FT = TEST::CalculateFullTransform1(itBones->path.node(&status));
                MMatrix FT2;
