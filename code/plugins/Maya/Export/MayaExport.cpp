...
BOOL CEditableObject::ParseMAMaterial(CSurface* dest, SXRShaderData& d)
{
    ...
    {
        Msg("!Empty shader name, material: %s", d.name.asChar());
        return FALSE;
    }
    ...
    // write faces
    {
        ...
        for (; !meshPoly.isDone(); meshPoly.next())
        {
            ...
            */
            ...
            {
                status = MS::kFailure;
				Msg("!polygon is missing UV information: %s", meshPoly.index());
                return status;
            }
        ...
        if ((MESH->GetVertexCount() < 4) || (MESH->GetFaceCount(true) < 2))
        {
...
LPCSTR CXRayObjectExport::getMaterialName(MDagPath& mdagPath, int cid, int objectIdx)
{
    ...
    if (!materialsEqual)
    {
        ...
        if (mLength == 0)
            Debug.fatal(DEBUG_INFO, "Object '%s' has polygon '%d' without material.", 0, cid);
        if (mLength > 1)
        {
