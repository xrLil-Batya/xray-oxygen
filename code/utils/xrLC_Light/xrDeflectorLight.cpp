...
// int PossibleRayCount = 0;
// if (!!(flags&LP_dont_rgb))
// {
// 	PossibleRayCount += Ligtings.rgb.size();
// }
// if (!!(flags&LP_dont_sun))
// {
// 	PossibleRayCount += Ligtings.sun.size();
// }
// if (!!(flags&LP_dont_hemi))
// {
// 	PossibleRayCount += Ligtings.hemi.size();
// }
// xr_vector<Ray> VertexRays;
// xr_vector<Ray_Detail> VertexRaysDetails;
// VertexRays.reserve(PossibleRayCount);
// VertexRaysDetails.reserve(PossibleRayCount);
// GetRaysFromVertex(Position, Normal, Ligtings, flags, VertexRays, VertexRaysDetails);
...
BOOL	compress_RMS(lm_layer& lm, u32 rms, u32& w, u32& h)
{
    ...
    if (w || h) {
        if (0 == w)	w = lm.width;
        if (0 == h)	h = lm.height;
        //		Logger.clMsg	("* RMS: [%d,%d] => [%d,%d]",lm.width,lm.height,w,h);
        return TRUE;
    }
    return FALSE;
}
