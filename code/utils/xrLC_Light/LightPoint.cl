...
#include "ConvertingMath.cl"
#include "base_color.h"

// Special to ISence
#ifdef __INTELLISENSE__
HardwareVector get_global_id();
#endif


__global void GetLightTypeAndIndex(xrHardwareLCGlobalData* GlobalData, u32 CurrentRayID,
	bool CheckRGB, bool CheckSun, bool CheckHemi, LightSource& LightType, int& LinearLightIndex)
{
	int LightID = -1;
...
__global bool GetRayOptimizeOut(xrHardwareLCGlobalData* GlobalData, RayRequest* RequestedRays, u32 SurfaceID, int LightID)
{
...
__global void CheckRayOptimizeOut(xrHardwareLCGlobalData* GlobalData, RayRequest* RequestedRays, char* StatusBuffer, u64 MaxPossibleRays, bool CheckRGB, bool CheckSun, bool CheckHemi)
{
	u64 idx = get_global_id().x;
...
__global void GetEnergyFromHit(xrHardwareLCGlobalData* GlobalData, Hit* InHit, float& TargetColor)
{
	...
	else
	{
		...
		int iU = convert_int_rtz(FinalU * float(SurfaceTex.Width) + 0.5f);
		int iV = convert_int_rtz(FinalV * float(SurfaceTex.Height) + 0.5f);
...
__global void ShiftRay(xrHardwareLCGlobalData* GlobalData, Ray* CurrentRay, Hit* InHit)
{
...
__global void ProcessHits(xrHardwareLCGlobalData* GlobalData, Ray* RayBuffer, Hit* HitBuffer,
...
__global void DoFinalizeRay(xrHardwareLCGlobalData* GlobalData, float InputEnergy, RayRequest& Request, R_Light& Light, LightSource InLightType, base_color_c& OutColor)
{
	switch (InLightType)
	{
	case LS_RGB:
...
__global void FinalizeRays(xrHardwareLCGlobalData* GlobalData, float* EnergyBuffer, RayRequest* RequestedRays, u32 RequestedRaysCount, base_color_c* OutColorBuffer, u32* AliveRayIndexes, u32 AliveRaysCount, bool CheckRGB, bool CheckSun, bool CheckHemi, int* SurfacePointStartLoc)
{
	int idx = get_global_id().x;
