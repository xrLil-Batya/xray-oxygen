...
void CDeflector::L_Direct_Edge (CDB::COLLIDER* DB, base_lighting* LightsSelected, Fvector2& p1, Fvector2& p2, Fvector& v1, Fvector& v2, Fvector& N, float texel_size, Face* skip)
{
	...
	for (int I=0; I<=steps; I++)
	{
		...
		LightPoint		(DB, inlc_global_data()->RCAST_Model(), C, P, N, *LightsSelected, ((inlc_global_data()->b_skiplmap() ? LP_dont_rgb : 0)|(inlc_global_data()->b_nosun())?LP_dont_sun:0)|LP_DEFAULT, skip); //.
...
//							else								
							...
							try {
								VERIFY(inlc_global_data());
								VERIFY(inlc_global_data()->RCAST_Model());
								LightPoint	(DB, inlc_global_data()->RCAST_Model(), C, wP, wN, *LightsSelected, ((inlc_global_data()->b_skiplmap() ? LP_dont_rgb : 0) | (inlc_global_data()->b_nosun()?LP_dont_sun:0))|LP_UseFaceDisable, F); //.
								Fcount		+= 1;
							} catch (...) {
