...
void CBuild::xrPhase_AdaptiveHT()
{
...
#if 0
	//DEBUG CODE START
	//"Vector" fuck you test level and you too.
	//Placing lights in a same level as floor is not good option, even if that work before (tests shows that GSC tracer got hits even on that lights. Mad world...)
	//I'm try to fix it at runtime
	Fvector FixingPosition;
	FixingPosition.set(0.0f, 1.5f, 0.0f);

	for (R_Light& RGBLight : StaticLighting.rgb)
	{
		RGBLight.position.add(FixingPosition);
	}
	...
	Logger.Status("Precalculating...");
	{
		...
		for (Vertex* V : lc_global_data()->g_vertices())
		{
			V->normalFromAdj();
		}
		...
		{
			Logger.Status("Setup OptiX scene ...");
			...
			Logger.Status("Calculate ...");

			LightCalculator.PerformAdaptiveHT();
#if 0
			xr_vector<Fvector> PCHitData = LightCalculator.GetDebugPCHitData();
			xr_vector<Fvector> GPUHitData = LightCalculator.GetDebugGPUHitData();
			xr_vector<PolyIndexes> TestMeshIndexesGPU;
			xr_vector<PolyIndexes> TestMeshIndexesCPU;

			for (int i = 0; i < GPUHitData.size(); i += 3)
			{
				PolyIndexes Ind{ i, i + 1, i + 2 };
				TestMeshIndexesGPU.push_back(Ind);
			}
			for (int i = 0; i < PCHitData.size(); i += 3)
			{
				PolyIndexes Ind{ i, i + 1, i + 2 };
				TestMeshIndexesCPU.push_back(Ind);
			}
		...
		else
		{
			...
			get_intervals(8, (u32)lc_global_data()->g_vertices().size(), threads, stride, rest);
...
void CBuild::u_SmoothVertColors(int count)
{
	for (int iteration = 0; iteration < count; ++iteration)
	{
		...
		u32 VertexesSize = (u32)lc_global_data()->g_vertices().size();
		...
		for (u32 it = 0; it < VertexesSize; ++it)
		{
			...
			Logger.Progress((ProgressSlice * float(iteration)) + (ProgressSlice * CurrentStageProgress));
		}
