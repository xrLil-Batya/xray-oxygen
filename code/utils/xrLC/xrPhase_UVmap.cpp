...
void CBuild::xrPhase_UVmap()
{
	...
	{
		Logger.Progress			(1.f * SP / g_XSplit.size());
...
void CBuild::mem_Compact()
{
	...
	LPCSTR h_Logger.Status		= 0;
	if (HeapValidate	(GetProcessHeap(),0,0))	h_Logger.Status = "OK";
	else										h_Logger.Status = "DAMAGED";
	Logger.clMsg				("::MEMORY(%s):: %d MB, %d Bused, %d Bfree",
		h_Logger.Status,bytes/(1024*1024),blocks_used,blocks_free);
	*/
}
