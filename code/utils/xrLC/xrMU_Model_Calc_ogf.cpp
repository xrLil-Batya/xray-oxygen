...
void calc_ogf( xrMU_Model &	mu_model )
{
	...
	for (xrMU_Model::v_subdivs_it it=mu_model.m_subdivs.begin(); it!=mu_model.m_subdivs.end(); it++)
	{
		...
		try {
			...
			} catch (...) {  Logger.clMsg("* ERROR: MU2OGF, model %s, *faces*",*(mu_model.m_name)); }
		} catch (...)
		{
		...
		try {
			pOGF->Optimize			();
		} catch (...)	{ Logger.clMsg	("* ERROR: MU2OGF, [optimize], model %s",*(mu_model.m_name)); }
		try {
			pOGF->CalcBounds		();
		} catch (...)	{ Logger.clMsg	("* ERROR: MU2OGF, [bounds], model %s",*(mu_model.m_name)); }
		try {
			pOGF->CalculateTB		();
		} catch (...)	{ Logger.clMsg	("* ERROR: MU2OGF, [calc_tb], model %s",*(mu_model.m_name)); }
		try {
			pOGF->MakeProgressive	(c_PM_MetricLimit_mu);
		} catch (...)	{ Logger.clMsg	("* ERROR: MU2OGF, [Logger.Progressive], model %s",*(mu_model.m_name)); }
		try {
			pOGF->Stripify			();
		} catch (...)	{ Logger.clMsg	("* ERROR: MU2OGF, [stripify], model %s",*(mu_model.m_name)); }
