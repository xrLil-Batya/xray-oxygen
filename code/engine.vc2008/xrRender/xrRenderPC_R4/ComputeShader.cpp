...
void ComputeShader::Dispatch(u32 dimx, u32 dimy, u32 dimz)
{
	...
	if (!m_Textures.empty())
		HW.pContext->CSSetShaderResources(0, (UINT)m_Textures.size(), &m_Textures[0]);

	if (!m_Samplers.empty())
		HW.pContext->CSSetSamplers(0, (UINT)m_Samplers.size(), &m_Samplers[0]);

	if (!m_Outputs.empty())
	{
		UINT num = 0;
		HW.pContext->CSSetUnorderedAccessViews(0, (UINT)m_Outputs.size(), &m_Outputs[0], &num);
	}
