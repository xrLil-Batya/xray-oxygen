...
void CParticleGroup::OnFrame(u32 InDeltaTime)
{
 	if (m_Def && m_RT_Flags.is(flRT_Playing))
 	{
		float fdeltaTime = float(InDeltaTime) / 1000.f;
        u32 iter = 0;
        ...
        {
        	i_it->OnFrame(InDeltaTime, *m_Def->m_Effects[i_it - items.begin()], box, bPlaying);
        }
