...
{
	if (is_backface(start_face_flags) != is_backface(test_face_flags))
		return false;

	return is_soft_edge(start_face_flags, start_common_edge_idx) && is_soft_edge(test_face_flags, test_common_edge_idx);
}
