...
float CExplosive::TestPassEffect(const	Fvector	&source_p,	const	Fvector	&dir,float range,float ef_radius,collide::rq_results& storage, CObject* blasted_obj)
{
	...
	if(range>EPS_L)
	{
		...
		SExpQParams			ep;
#endif
		g_pGameLevel->ObjectSpace.RayQuery(storage,RD, (collide::rq_callback*)grenade_hit_callback,&ep,nullptr,blasted_obj);
...
void CExplosive::ExplodeWaveProcess()
{
	...
	while (!m_blasted_objects.empty() && 0!=i) {
...
void CExplosive::net_Relcase(CObject* O)
{
    ...
    auto I=std::find(m_blasted_objects.begin(),m_blasted_objects.end(),smart_cast<CPhysicsShellHolder*>(O));
