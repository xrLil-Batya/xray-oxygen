...
void CALifeRegistryContainer::load(IReader &file_stream)
{
	...
	process<TYPE_LIST>([&](auto item) 
	{
		...
		if constexpr (object_type_traits::is_base_and_derived_v<IPureLoadableObject<IReader>, type>) {this->type::load(file_stream);}
	});
}

void CALifeRegistryContainer::save(IWriter &memory_stream)
{
	memory_stream.open_chunk(REGISTRY_CHUNK_DATA); process<TYPE_LIST>([&](auto item)
	{
		using type = typename decltype(item)::type;
		if constexpr (object_type_traits::is_base_and_derived_v<IPureSavableObject<IWriter>, type>) { this->type::save(memory_stream); }
	});
