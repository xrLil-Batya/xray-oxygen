...
void CAI_PhraseDialogManager::AnswerPhrase(DIALOG_SHARED_PTR& phrase_dialog)
{
	...
	if (!phrase_dialog->IsFinished())
	{
		...
		size_t phrase_num = phrase_dialog->PhraseList().size() - 1;
		...
		phrase_num = size_t(phrases[Random.randI(0, (u32)phrases.size())]);

		shared_str phrase_id = phrase_dialog->PhraseList()[phrase_num]->GetID();

		GameUI()->TalkMenu->AddAnswer(phrase_dialog->GetPhraseText(phrase_id), pInvOwner->Name());
