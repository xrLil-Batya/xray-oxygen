...
{
	// #TODO: Correct price for non-artefact objects
	...
	if (l_tpALifeItemArtefact)
	{
		...
		// #TODO: Optimize this cycle by keeping additional data structure with bought items
		for (u16 id : children)
		{
			if (!xr_strcmp(ai().alife().objects().object(id)->s_name, l_tpALifeItemArtefact->s_name))
				++l_dwPurchasedCount;
		}
	}
