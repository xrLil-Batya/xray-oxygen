...
// Wound size to start particles
float CEntityAlive::m_fStartBurnWoundSize = 0.3f;
// Wound size to stop particles
...
void CEntityAlive::Load(LPCSTR section)
{
	...
	// The biological species to which the monster or character belongs
...
void CEntityAlive::LoadBloodyWallmarks (LPCSTR section)
{
	...
	for (int i = 0; i < cnt; ++i)
		(*m_pBloodMarksVector)->AppendMark(_GetItem(wallmarks_name, i, tmp));
	...
	// Blood drops from open wounds
	wallmarks_name = pSettings->r_string(section, "blood_drops");

	cnt	=_GetItemCount(wallmarks_name);
	for (int i = 0; i < cnt; ++i)
		(*m_pBloodDropsVector)->AppendMark(_GetItem(wallmarks_name, i, tmp));
...
void CEntityAlive::LoadFireParticles(LPCSTR section)
{
	...
	for (int i = 0; i < cnt; ++i)
	{
		shared_str s = _GetItem(particles_name, i, tmp);
...
void CEntityAlive::Hit(SHit* pHDS)
{
	...
	// Change state before parent objects process hit
...
void CEntityAlive::Die(CObject* who)
{
	...
	const CGameObject *pWhoGameObj = smart_cast<const CGameObject*>(who);
	callback(GameObject::eDeath)(lua_game_object(), pWhoGameObj ? pWhoGameObj->lua_game_object() : nullptr);
...
// Called while hit calculated
float CEntityAlive::CalcCondition(float /**hit/**/)
{	
	conditions().UpdateCondition();

	// Don't call inherited::CalcCondition it will be meaningless
...
// Add bloody marks on walls after hit received
void CEntityAlive::BloodyWallmarks(float P, const Fvector &dir, s16 element, const Fvector& position_in_object_space)
{
	...
	if (pV)
	{
		Fmatrix& m_bone = (pV->LL_GetBoneInstance(u16(element))).mTransform;
		m_bone.transform_tiny(start_pos);
	}
	XFORM().transform_tiny(start_pos);

	float small_entity = 1.0f;
...
void CEntityAlive::PlaceBloodWallmark(const Fvector& dir, const Fvector& start_pos, float trace_dist, float wallmark_size, IWallMarkArray *pwallmarks_vector)
{
	...
	bool bSurfaceReached = Level().ObjectSpace.RayPick(start_pos, dir, trace_dist, collide::rqtBoth, result, this);
...
void CEntityAlive::StartFireParticles(CWound* pWound)
{
	if (pWound->TypeSize(ALife::eHitTypeBurn) > m_fStartBurnWoundSize)
	{
		...
		int psLifeTime = int(float(m_dwMinBurnTime)*::Random.randF(0.5f, 1.5f));

		if (BI_NONE != particle_bone)
			CParticlesPlayer::StartParticles(pWound->GetParticleName(), pWound->GetParticleBoneNum(), Fvector().set(0, 1, 0), ID(), psLifeTime, false);
		else
			CParticlesPlayer::StartParticles(pWound->GetParticleName(), Fvector().set(0, 1, 0), ID(), psLifeTime, false);
	}
}
...
//	WOUND_VECTOR_IT last_it;
	...
	for (auto it = m_ParticleWounds.begin(); it != m_ParticleWounds.end();)
	{
		...
		{
			int psLifeTime = int(float(m_dwMinBurnTime)*::Random.randF(0.5f, 1.5f));
			CParticlesPlayer::AutoStopParticles(pWound->GetParticleName(), pWound->GetParticleBoneNum(), psLifeTime);
...
{
	ALife::ERelationType relation = tfGetRelationType(tpEntityAlive);
	...
	for(auto it = m_BloodWounds.begin(); it != m_BloodWounds.end();)
	{
		...
		if (pWound->m_fDropTime < Device.fTimeGlobal)
		{
			...
			if (pWound->GetBoneNum() != BI_NONE)
			{
				...
				pos.add(pos_distort);
				PlaceBloodWallmark(dir, pos, m_fBloodMarkDistance, m_fBloodDropSize, &**m_pBloodDropsVector);
			}
		}
		++it;
	}
...
void CEntityAlive::create_anim_mov_ctrl(CBlend *b, Fmatrix *start_pose, bool local_animation)
{
	...
	CCharacterPhysicsSupport *pCPS = character_physics_support();
	if (!b_animation_movement_controlled && pCPS)
		pCPS->on_create_anim_mov_ctrl();
}
...
Fvector	CEntityAlive::get_new_local_point_on_mesh	( u16& bone_id ) const
{
	...
	float hit_bones_surface_area = 0.0f;

	for (auto &boneData : m_hit_bone_surface_areas)
	{
		if (!kinematics->LL_GetBoneVisible(boneData.first))
			continue;

		SBoneShape const& shape = kinematics->LL_GetData(boneData.first).shape;
