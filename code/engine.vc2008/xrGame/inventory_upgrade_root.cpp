...
void Root::construct( const shared_str& root_id, Manager& manager_r )
{
	...
	VERIFY_FORMAT( upgrade_scheme_str, "In inventory item <%s> `upgrade_scheme` is empty!", root_id.c_str());
...
void Root::add_upgrade( Upgrade* upgr )
{
	...
	VERIFY_FORMAT( verify_scheme_index( scheme_index ),
		"in upgrade <%s> for item <%s> scheme index [%d, %d] is duplicated !",
