...
void moving_bones_snd_player::load(IKinematics &K, CInifile& ini, LPCSTR section, const Fmatrix &object)
{
	...
	VERIFY_FORMAT(min_factor > 0.f, "moving_bones_snd_player: bad params: min_factor: %f < 0 ", min_factor);
	VERIFY_FORMAT(max_factor > 0.f, "moving_bones_snd_player: bad params: max_factor: %f < 0 ", max_factor);
	VERIFY_FORMAT(max_factor > 0.f, "moving_bones_snd_player: bad params: base_velocity: %f < 0 ", base_velocity);
	VERIFY_FORMAT(min_factor <= max_factor, "moving_bones_snd_player: bad params: min_factor %f > max_factor %f, ", min_factor, max_factor);
