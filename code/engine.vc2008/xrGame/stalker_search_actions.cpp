...
CStalkerActionReachEnemyLocation::CStalkerActionReachEnemyLocation(CAI_Stalker *pObject, CPropertyStorage *pStorage, LPCSTR ActionName) :
	inherited(pObject, ActionName),
	m_combat_storage(pStorage)
{
}
...
void CStalkerActionReachEnemyLocation::execute()
{
	...
	if (object().movement().path_completed()) 
	{
		...
		{
			object().movement().
				set_nearest_accessible_position(mem_object.m_object_params.m_position, mem_object.m_object_params.m_level_vertex_id);
		}

		object().sight().setup(CSightAction(SightManager::eSightTypePosition,
			Fvector(mem_object.m_object_params.m_position).add(Fvector().set(0.f, .5f, 0.f)),true));
