...
void loophole::fill_transitions(luabind::object const & transitions_table)
{
	...
	for ( ; I != E; ++I) {
		...
		if (!m_transitions.vertex(action_to))
			m_transitions.add_vertex	(imdexlib::empty(), action_to);
...
smart_cover::action::Animations const &loophole::action_animations(shared_str const &action_id, shared_str const &animation_id) const
{
	...
	VERIFY_FORMAT(found != m_actions.end(), "action [%s] not present in loophole [%s]", action_id.c_str(), m_id.c_str());
...
loophole::TransitionData const &loophole::transition_animations	(shared_str const &action_from, shared_str const &action_to) const
{
	...
	VERIFY_FORMAT(edge, "transition [%s]->[%s] absent", action_from.c_str(), action_to.c_str());
	return					(edge->data());
}
