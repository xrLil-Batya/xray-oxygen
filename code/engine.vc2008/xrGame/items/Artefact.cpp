...
void CArtefact::Load(LPCSTR section) 
{
	...
	if (m_bLightsEnabled)
	{
		...
		m_bTrailLightShadow			= !!pSettings->r_bool(section, "trail_light_shadow");
		m_bTrailLightVolumetric		= !!pSettings->r_bool(section, "trail_light_volumetric");
		m_fTrailLightRange			= pSettings->r_float(section, "trail_light_range");
		m_fTrailLightVirtualSize	= pSettings->r_float(section, "trail_light_virtual_size");
	}
...
void SArtefactDetectorsSupport::SetVisible(bool b)
{
	...
	if (b)
	{
		...
		// Play show/hide particles
		LPCSTR currentParticles = pSettings->r_string(m_parent->cNameSect().c_str(), (b) ? "det_show_particles" : "det_hide_particles");
		...
		m_parent->CParticlesPlayer::StartParticles(currentParticles, bone_id, Fvector().set(0, 1, 0), m_parent->ID());

		// Play show/hide sound
		LPCSTR currentSound = pSettings->r_string(m_parent->cNameSect().c_str(), (b) ? "det_show_snd" : "det_hide_snd");
		m_sound.create(currentSound, st_Effect, sg_SourceType);
...
void SArtefactDetectorsSupport::UpdateOnFrame()
{
	if (m_currPatrolPath && !m_parent->getVisible())
	{
		...
		float cos_et = _cos(deg2rad(45.0f));
		Fvector dir;
...
#pragma todo("RZ: check if we need to replace 10 by current time factor.")
