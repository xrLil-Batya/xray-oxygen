...
void CControlManagerCustom::load_jump_data(LPCSTR s1, LPCSTR s2, LPCSTR s3, LPCSTR s4, u32 vel_mask_prepare, u32 vel_mask_ground, u32 flags)
{
	...
	if (s2) {
		...
		m_jump->setup_data().flags.maskOr(SControlJumpData::ePrepareInMove);
	...
	if (s4) {
		...
		m_jump->setup_data().flags.maskOr(SControlJumpData::eGroundSkip);
	}

	if (!s1 && !s2) {
		m_jump->setup_data().flags.maskOr(SControlJumpData::ePrepareSkip);
	}
	
	m_jump->setup_data().flags.maskOr(SControlJumpData::eGlidePlayAnimOnce);
	m_jump->setup_data().flags.maskOr(SControlJumpData::eGlideOnPrepareFailed);
...
void CControlManagerCustom::jump(const Fvector &position)
{
	...
	ctrl_data->flags.maskOr								(SControlJumpData::ePrepareSkip);
	ctrl_data->force_factor							= -1.f;
