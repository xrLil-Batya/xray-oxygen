...
CDamagableItem::CDamagableItem() : m_max_health(0.f), m_levels_num(u16(-1)), m_level_applied(u16(-1)), m_health(0.f)
{
}

u16 CDamagableItem::DamageLevel()
{
	const float &refHealth = Health() < 0 ? 0 : Health();

	u16 dl = u16((1.f - refHealth / m_max_health)*m_levels_num);
...
void CDamagableItem::Hit(float P)
{
