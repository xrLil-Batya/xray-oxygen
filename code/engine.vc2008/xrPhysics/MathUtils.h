...
inline  float* cast_fp(Fvector& fv) { return (float*)(&fv); }

inline const float* cast_fp(const Fvector& fv) { return (const float*)(&fv); }

inline Fvector &cast_fv(float* fp) { return *((Fvector*)fp); }

inline const Fvector &cast_fv(const float* fp) { return *((const Fvector*)fp); }

inline  float dXZMag(const float* v) { return _sqrt(v[0] * v[0] + v[2] * v[2]); }

inline float dXZMag(const Fvector &v) { return dXZMag(cast_fp(v)); }

inline	float	dXZDot(const float* v0, const float* v1) { return v0[0] * v1[0] + v0[2] * v1[2]; }

inline	float	dXZDotNormalized(const Fvector& v0, const Fvector& v1) { return (v0.x*v1.x + v0.z*v1.z) / _sqrt((v0.x*v0.x + v0.z*v0.z)*(v1.x*v1.x + v1.z*v1.z)); }

inline	float	dXZDotNormalized(const float* v0, const float* v1) { return dXZDotNormalized(cast_fv(v0), cast_fv(v1)); }

inline	float	dXZDot(const Fvector& v0, const Fvector& v1) { return v0.x*v1.x + v0.z*v1.z; }
inline	void	dVectorSet(float* vd, const float* vs)
{
	vd[0] = vs[0];
	vd[1] = vs[1];
	vd[2] = vs[2];
...
inline	void	dVectorSetInvert(float* vd, const float* vs)
{
	vd[0] = -vs[0];
	vd[1] = -vs[1];
	vd[2] = -vs[2];
...
inline	void dVectorSetZero(float* vd)
{
	vd[0] = 0.f;
	vd[1] = 0.f;
	vd[2] = 0.f;
...
inline	void	dVector4Set(float* vd, const float* vs)
{
	vd[0] = vs[0];
	vd[1] = vs[1];
	vd[2] = vs[2];
...
inline	void	dVector4SetZero(float* vd)
{
	vd[0] = 0.f;
	vd[1] = 0.f;
	vd[2] = 0.f;
...
inline void		dQuaternionSet(float* vd, const float* vs)
{
	dVector4Set(vd, vs);
}

inline	void	dVectorAdd(float* v, const float* av)
{
	v[0] += av[0];
	v[1] += av[1];
	v[2] += av[2];
}
inline	void	dVectorAddMul(float* v, const float* ad, float mul)
{
	v[0] += ad[0] * mul;
	v[1] += ad[1] * mul;
	v[2] += ad[2] * mul;
}
inline	void	dVectorAdd(float* v, const float* av0, const float* av1)
{
	v[0] = av0[0] + av1[0];
	v[1] = av0[1] + av1[1];
	v[2] = av0[2] + av1[2];
...
inline	void	dVectorSub(float* v, const float* av)
{
	v[0] -= av[0];
	v[1] -= av[1];
	v[2] -= av[2];
...
inline	void	dVectorSub(float* v, const float* av1, const float* av0)
{
	v[0] = av1[0] - av0[0];
	v[1] = av1[1] - av0[1];
	v[2] = av1[2] - av0[2];
}
inline	void	dVectorInvert(float* v)
{
	v[0] = -v[0];
	v[1] = -v[1];
	v[2] = -v[2];
}
inline	void	dVectorMul(float* v, float mt)
{
	v[0] *= mt;
	v[1] *= mt;
	v[2] *= mt;
}
inline	void	dVectorMul(float* res, const float* av, float mt)
{
	res[0] = av[0] * mt;
	res[1] = av[1] * mt;
	res[2] = av[2] * mt;
}
inline	void	dVectorInterpolate(float* v, float* to, float k) //changes to
{
	dVectorMul(v, 1.f - k); dVectorMul(to, k); dVectorAdd(v, to);
}

inline	void	dVectorDeviation(const float* vector3_from, const float* vector3_to, float* vector_dev)
{
	dVectorSub(vector_dev, vector3_to, vector3_from);
}

inline	void	dVectorDeviationAdd(const float* vector3_from, const float* vector3_to, float* vector_dev)
{
...
inline	void	dMatrixSmallDeviation(const float* matrix33_from, const float* matrix33_to, float* vector_dev)
{
...
inline	void	dMatrixSmallDeviationAdd(const float* matrix33_from, const float* matrix33_to, float* vector_dev)
{
	vector_dev[0] += matrix33_from[10] - matrix33_to[10];
	vector_dev[1] += matrix33_from[2] - matrix33_to[2];
	vector_dev[2] += matrix33_from[4] - matrix33_to[4];
...
inline void		prg_on_normal(const Fvector	&in_norm, const Fvector &in_dir, Fvector &out_dir)
{
...
inline bool check_obb_sise(const Fobb& obb)
{
