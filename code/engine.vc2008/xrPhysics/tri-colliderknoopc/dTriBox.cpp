...
#include "../../3rd-party/ode/include/oxy/dReal.hpp"
...
IC bool normalize_if_possible(dReal *v)
{
	...
	xray::ode::dUp(v, dRecipSqrt(sqr_magnitude));
	return true;
}
...
#define TEST(sd, c) \
			\
			xray::ode::dSet(pDist, dDOT14(v0,R+sd)-dDOT14(p,R+sd));\
			\
			isPdist0=pDist[0]>0.f;\
			isPdist1=pDist[1]>0.f;\
			isPdist2=pDist[2]>0.f;\
			\
			depth0=hside[sd]-dFabs(pDist[0]);\
			depth1=hside[sd]-dFabs(pDist[1]);\
			depth2=hside[sd]-dFabs(pDist[2]);\
			\
			test0##sd = depth0>0.f;\
			test1##sd = depth1>0.f;\
			test2##sd = depth2>0.f;\
...
\
	pDist[ax] = dDOT(v##ax,axis)-dDOT(p,axis);\
	pDist[ox] = dDOT(v##ox,axis)-dDOT(p,axis);\
\
isPdist##ax=pDist[ax]>0.f;\
isPdist##ox=pDist[ox]>0.f;\
if(isPdist##ax != isPdist##ox) continue;\
\
depth##ax=sidePr-dFabs(pDist[ax]);\
depth##ox=sidePr-dFabs(pDist[ox]);\
	if(depth##ax>depth##ox){\
			if(depth##ax>0.f){\
				if(depth##ax*1.05f<outDepth) \
					{\
						dReal sgn=pDist[ax]<0.f ? -1.f : 1.f;\
						dReal sgn1=sgn*dDOT14(axis,R+ix1)<0.f ? -1.f : 1.f;\
						dReal sgn2=sgn*dDOT14(axis,R+ix2)<0.f ? -1.f : 1.f;\
						for(u32 ii=0;ii<3;++ii) crpos[ii]=p[ii]+R[ii*4+ix1]*hside[ix1]*sgn1+R[ii*4+ix2]*hside[ix2]*sgn2;\
...
#undef TEST
	...
	if (code == 0) 
	{
		xray::ode::dSet(norm, triAx, signum);
