...
dMass CPHElement::recursive_mass_summ(u16 start_geom, FRACTURE_I cur_fracture)
{
	...
	GEOM_I i_geom	= m_geoms.begin() + start_geom, 
		   end_geom = m_geoms.begin() + cur_fracture->m_start_geom_num;

	for (; i_geom != end_geom; ++i_geom)
	{
...
void CPHElement::Deactivate()
{
	...
	IKinematics* pShellKinematic = m_shell->PKinematics();
	if (pShellKinematic)
	{
		if (pShellKinematic->LL_GetBoneInstance(m_SelfID).callback_type() == bctPhysics)
			ClearBoneCallback();
	}
}
...
void CPHElement::Update() {
	...
	if (!dBodyIsEnabled(m_body) && !m_flags.test(flUpdate)) return;
...
void CPHElement::PhDataUpdate(dReal step) 
{
	...
	if (!m_flags.test(flEnabledOnStep)) return;
	...
	//force/velocity !!!
	dReal l_air = linear_velocity_mag * k_l;
...
void CPHElement::calculate_it_data_use_density(const Fvector& mc, float density)
{
	...
	for (CODEGeom* it_geom: m_geoms)
	{
		it_geom->add_self_mass(m_mass, mc, density);
	}
...
{
	for (CODEGeom* iter: m_geoms)
	{
		iter->move_local_basis(shift_pivot);
	}
...
void CPHElement::ReInitDynamics(const Fmatrix &shift_pivot, float density)
{
	...
	for (CODEGeom *iter: m_geoms)
	{
		iter->set_build_position(m_mass_center);
		iter->set_body(m_body);
		group_add(*iter);
	}
}
