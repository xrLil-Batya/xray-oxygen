...
inline BOOL dV_valid(const dReal * v)
{
...
{
	return  _valid(m[0]) && _valid(m[1]) && _valid(m[2]) && _valid(m[4]) && _valid(m[5]) && _valid(m[6]) && _valid(m[8]) && _valid(m[9])
		&& _valid(m[10]);
}
...
inline BOOL dQ_valid(const dReal* q)
{
	return dV4_valid(q);
}
inline BOOL dMass_valide(const dMass* m)
{
