...
static int llex(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
    ...
    case '<':
      next(ls);
	  if (ls->current == '=') { next(ls); return TK_le; } else
	  if (ls->current == '<') { next(ls); return TK_shl; } else return '<';
    case '>':
      next(ls);
	  if (ls->current == '=') { next(ls); return TK_ge; } else
	  if (ls->current == '>') { next(ls); return TK_shr; } else return '>';
	case '/':
		next(ls);
	  if (ls->current != '/') return '/'; else { next(ls); return TK_idiv; }
    case '~':
      next(ls);
