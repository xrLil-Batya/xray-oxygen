...
static int build_code(BuildCtx *ctx)
{
	...
	ctx->nrelocsym = 0;
	for (int i = 0; i < (int)NRELOCSYM; i++) relocmap[i] = -1;
	...
	for (int i = 0; i < ctx->npc; i++) 
	{
		int32_t ofs = dasm_getpclabel(Dst, i);
		if (ofs < 0) return 0x22000000 | i;
		ctx->bc_ofs[i] = ofs;
		if ((LJ_HASJIT ||
			!(i == BC_JFORI || i == BC_JFORL || i == BC_JITERL || i == BC_JLOOP || i == BC_IFORL || i == BC_IITERL || i == BC_ILOOP)) &&
...
int main(int argc, char **argv)
{
	...
	binmode = (ctx->mode == BUILD_peobj) ? 1 : 0;
	...
	if (ctx->mode == BUILD_peobj)
	{
		emit_peobj(ctx);
	}
	else
	{
		fprintf(stderr, "Error: error using BuildVM.");
		exit(1);
	}
