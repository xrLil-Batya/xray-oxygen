...
LPSTR strconcat(int dest_sz, char* dest, const char argv[])
{
	VERIFY(dest);
	VERIFY(argv);
	char* strings = new char[strlen(argv)];

	for(auto i = 0; i < strlen(argv); i++)
		strings[i] = argv[i];

	LPSTR			i = dest;
	LPCSTR			e = dest + dest_sz;
	char*			j;
	for (j = (char*)strings[0]; *j && i < e; ++i, ++j)
		*i = *j;
	LPCSTR a = (LPCSTR)strings[0];
	strconcat_error::process(i, e, 0, a);
