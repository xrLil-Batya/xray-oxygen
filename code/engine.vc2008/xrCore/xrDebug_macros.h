...
#define R_ASSERT_FORMAT(expr, format, ...) do {static bool ignore_always = false; if (!ignore_always && !(expr)) {string1024 msg; xr_sprintf(msg, format, __VA_ARGS__); ::Debug.fail(_TRE(#expr), msg, DEBUG_INFO,ignore_always);}} while(0)
