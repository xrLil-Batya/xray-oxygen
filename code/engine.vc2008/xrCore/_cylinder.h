...
public:
	...
	T m_radius;
public:
	inline SelfRef	invalidate() { m_center.set(0, 0, 0); m_direction.set(0, 0, 0); m_height = 0; m_radius = 0; return *this; }
	...
	inline int intersect(const _vector3<T>& start, const _vector3<T>& dir, T afT[2], ecode code[2]) const
	{
		...
		if (_abs(kD.z) <= fEpsilon) {
			...
			if (fDiscr < 0.0f)// line does not intersect cylinder wall
				return 0;
			else if (fDiscr > 0.0f)
			{
				fRoot = _sqrt(fDiscr);
		...
		if (iQuantity == 2) // line intersects both top and bottom
			return 2;//both caps
...
template <class T>
BOOL _valid(const _cylinder<T>& c) { return _valid(c.m_center) && _valid(c.m_direction) && _valid(c.m_height); }
