...
inline u32	xr_strlen(const shared_str & a)	throw() { return a.size(); }
inline int	xr_strcmp(const shared_str & a, const char* b) 	throw() { return xr_strcmp(*a, b); }
inline int	xr_strcmp(const char* a, const shared_str & b)	throw() { return xr_strcmp(a, *b); }

inline int	xr_strcmp(const shared_str & a, const shared_str & b) throw()
{
	if (a.equal(b))
		return 0;
	else
