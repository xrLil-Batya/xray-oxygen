...
struct xr_token;
	...
	XRCORE_API 	const char*		_GetItems 				( const char* src, int idx_start, int idx_end, AnsiString& dst, char separator );
	...
	XRCORE_API 	AnsiString&	_ReplaceItem 			( const char* src, int index, const char* new_item, AnsiString& dst, char separator );
	XRCORE_API 	AnsiString&	_ReplaceItems 			( const char* src, int idx_start, int idx_end, const char* new_items, AnsiString& dst, char separator );
...
template <int count>
inline char*	    		_GetItem				( const char* src, int index, char (&dst)[count], char separator=',', const char* def="", bool trim=true )
{
