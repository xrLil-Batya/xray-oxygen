...
namespace tinyxml2
{
		...
		if (*_start)
		{
			...
			while (*p) {
				if (XMLUtil::IsWhiteSpace(*p))
				{
					...
					if (*p == 0)// don't write to q; this trims the trailing space.
						break;
	...
	*/
	...
	XMLNode* XMLNode::InsertAfterChild(XMLNode* afterThis, XMLNode* addThis)
	{
		...
		if (afterThis->_next == nullptr) // The last node or the only node.
	...
	char* XMLNode::ParseDeep(char* p, StrPair* parentEnd)
	{
		...
		while (p && *p)
		{
			...
			if (ele)
			{
				...
				if (mismatch)
				{
					_document->SetError(XML_ERROR_MISMATCHED_ELEMENT, ele->Name(), nullptr);
					DeleteNode(node);
					break;
				}
			}
	...
	XMLError XMLDocument::SaveFile(FILE* fp, bool compact)
	{
		...
		// for *this* call.
		SetError(XML_SUCCESS, nullptr, nullptr);
	...
	XMLPrinter::XMLPrinter(FILE* file, bool compact, int depth) :_elementJustOpened(false), _firstElement(true), _fp(file), _depth(depth), _textDepth(-1),
		...
		_compactMode(compact)
	{
		...
		for (const Entity &entitie : entities)
		{
