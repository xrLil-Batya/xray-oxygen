...
//------------------------------------------------------------------------------
XRCORE_API BOOL _parse(char* dest, const char* src) {
...
XRCORE_API void _decorate(char* dest, const char* src) 
{
    if (src) 
    {
        bool bInsideSTR = false;
        while (*src)
	{
		...
		else if (*src == '"') bInsideSTR = !bInsideSTR;
		*dest++ = *src++;
	}
    }
    *dest = 0;
...
std::string CInifile::r_string_wbStd(LPCSTR S, LPCSTR L) const {
	...
	if ('"' == _original[_len - 1])
		_original[_len - 1] = 0; // skip end
	if ('"' == _original[0])
		return std::string(&_original[0] + 1); // skip begin
	return std::string(_original);
}
