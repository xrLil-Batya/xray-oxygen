...
{
	u32 hours = return_time(g_pGameLevel->GetGameTime(), ETimeType::eHOURS);
	...
	m_UIStaticItem.SetSize(Fvector2().set((float)iFloor((end_pixel-start_pixel)*UI().get_current_kx()), 128.f));
	m_UIStaticItem.SetPos(pos.x, pos.y);
	if(end_pixel2>0)
	{
		r.set((float)start_pixel2, 0.0f, (float)end_pixel2, 128.0f);
		m_UIStaticItem2.SetTextureRect(r);
		m_UIStaticItem2.SetSize(Fvector2().set((float)iFloor(end_pixel2*UI().get_current_kx()), 128.f));
