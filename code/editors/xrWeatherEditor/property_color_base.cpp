...
{
	return				(m_holder->GetValue_raw().r);
}
...
{
	return				(m_holder->GetValue_raw().g);
}
...
{
	return				(m_holder->GetValue_raw().b);
}
...
{
	editor::color		current = GetValue_raw();
	current.r			= value;
	SetValue_raw		(current);
}
...
void property_color_base::OnDoubleClick	(XRay::SdkControls::PropertyGrid^ property_grid)
{
	...
	editor::color				color = GetValue_raw();
...
void property_color_base::Increment(float increment)
{
	...
	editor::color				value = GetValue_raw();
