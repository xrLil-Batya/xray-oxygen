...
#ifdef DEBUG
IDirect3DStateBlock9* dwDebugSB = NULL;
#endif
...
void CHW::CreateDevice(HWND m_hWnd, bool move_window)
{
	...
	R_CHK(pD3D->GetAdapterIdentifier(DevAdapter, NULL, &adapterID));
	...
	// Create the device
	u32 GPU = selectGPU();
	HRESULT R = HW.pD3D->CreateDevice(DevAdapter, DevT, m_hWnd, GPU | D3DCREATE_MULTITHREADED /* locks at present */, &P, &pDevice);

	if (FAILED(R))
	{
		R = HW.pD3D->CreateDevice(DevAdapter, DevT, m_hWnd, GPU | D3DCREATE_MULTITHREADED,	/* locks at present */&P, &pDevice);
	}
...
void CHW::updateWindowProps(HWND m_hWnd)
{
	...
	if (bWindowed)
	{
		if (m_move_window)
		{
			...
			SetWindowPos(m_hWnd, HWND_NOTOPMOST, m_rcWindowBounds.left, m_rcWindowBounds.top, (m_rcWindowBounds.right - m_rcWindowBounds.left),
...
struct _uniq_mode
{
	_uniq_mode(const char* v) :_val(v) {}
	const char* _val;
	bool operator() (const char* _other) { return !stricmp(_val, _other); }
};
