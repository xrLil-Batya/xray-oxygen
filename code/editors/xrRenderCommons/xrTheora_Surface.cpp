...
#ifndef _EDITOR
		IRenderDevice* RealDevice = dynamic_cast <IRenderDevice*> (RenderDevice);
		CHECK_OR_EXIT(RealDevice != nullptr, "Failed to obtain render device");
		R_ASSERT(RealDevice->m_pRender);
		bShaderYUV2RGB = RealDevice->m_pRender->HWSupportsShaderYUV2RGB();
#else // _EDITOR
