...
void Image_DXTC::DecompressDXT1()
{
	...
	{
		// 8 bytes per block
		pBlock = (DXTColBlock*)((char*)m_pCompBytes + j * xblocks * 8);
...
void Image_DXTC::DecompressDXT3()
{
	...
	for (int j = 0; j < yblocks; j++)
	{
		...
		pBlock = (DXTColBlock*)((char*)m_pCompBytes + j * xblocks * 16);
		for (int i = 0; i < xblocks; i++, pBlock++)
		{
			// inline
			// Get alpha block
...
void Image_DXTC::DecompressDXT5()
{
	...
	for (int j = 0; j < yblocks; j++)
	{
		// 8 bytes per block
		// 1 block for alpha, 1 block for color
		DXTColBlock* pBlock = (DXTColBlock*)((char*)m_pCompBytes + j * xblocks * 16);
		for (int i = 0; i < xblocks; i++, pBlock++)
		{
			// inline
			// Get alpha block
