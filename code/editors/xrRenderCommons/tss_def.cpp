...
#include "../xrRenderDX10/DirectXStateUtils.h"

void SimulatorStates::UpdateState(DirectXState &state) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
...
void SimulatorStates::UpdateDesc(D3D_RASTERIZER_DESC &desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 0)
		{
			...
			case D3DRS_CULLMODE:
				desc.CullMode = DirectXStateUtils::ConvertCullMode((D3DCULL)S.v2);
				break;
				/*
				switch (S.v2)
				{
...
void SimulatorStates::UpdateDesc(D3D_DEPTH_STENCIL_DESC &desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 0)
		{
			...
			case D3DRS_ZFUNC:
				desc.DepthFunc = DirectXStateUtils::ConvertCmpFunction((D3DCMPFUNC)S.v2);
				break;
			...
			case D3DRS_STENCILFAIL:
				desc.FrontFace.StencilFailOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_STENCILZFAIL:
				desc.FrontFace.StencilDepthFailOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_STENCILPASS:
				desc.FrontFace.StencilPassOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_STENCILFUNC:
				desc.FrontFace.StencilFunc = DirectXStateUtils::ConvertCmpFunction((D3DCMPFUNC)S.v2);
				break;

			case D3DRS_CCW_STENCILFAIL:
				desc.BackFace.StencilFailOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_CCW_STENCILZFAIL:
				desc.BackFace.StencilDepthFailOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_CCW_STENCILPASS:
				desc.BackFace.StencilPassOp = DirectXStateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
				break;

			case D3DRS_CCW_STENCILFUNC:
				desc.BackFace.StencilFunc = DirectXStateUtils::ConvertCmpFunction((D3DCMPFUNC)S.v2);
				break;
			}
		}
	}
...
void SimulatorStates::UpdateDesc(D3D_BLEND_DESC &desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 0)
		{
			...
			case D3DRS_SRCBLEND:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].SrcBlend = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLEND:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].DestBlend = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;
			...
			case D3DRS_BLENDOP:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].BlendOp = DirectXStateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;

			case D3DRS_SRCBLENDALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].SrcBlendAlpha = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLENDALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].DestBlendAlpha = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_BLENDOPALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].BlendOpAlpha = DirectXStateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;
...
void SimulatorStates::UpdateDesc(D3D_BLEND_DESC &desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 0)
		{
			...
			case D3DRS_SRCBLEND:
				desc.SrcBlend = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLEND:
				desc.DestBlend = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

				//D3DRS_ALPHAFUNC

			case D3DRS_BLENDOP:
				desc.BlendOp = DirectXStateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;

			case D3DRS_SRCBLENDALPHA:
				desc.SrcBlendAlpha = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLENDALPHA:
				desc.DestBlendAlpha = DirectXStateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_BLENDOPALPHA:
				desc.BlendOpAlpha = DirectXStateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;
	...
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 2)
		{
			...
			case D3DSAMP_ADDRESSU:	/* D3DTEXTUREADDRESS for U coordinate */
				desc.AddressU = DirectXStateUtils::ConvertTextureAddressMode(D3DTEXTUREADDRESS(S.v3));
				break;

			case D3DSAMP_ADDRESSV:	/* D3DTEXTUREADDRESS for V coordinate */
				desc.AddressV = DirectXStateUtils::ConvertTextureAddressMode(D3DTEXTUREADDRESS(S.v3));
				break;

			case D3DSAMP_ADDRESSW:	/* D3DTEXTUREADDRESS for W coordinate */
				desc.AddressW = DirectXStateUtils::ConvertTextureAddressMode(D3DTEXTUREADDRESS(S.v3));
				break;
