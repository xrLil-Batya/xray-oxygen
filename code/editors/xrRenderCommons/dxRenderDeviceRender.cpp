...
dxRenderDeviceRender* g_RDRinstance = 0;
...
void dxRenderDeviceRender::overdrawEnd()
{
	...
	for (int I = 0; I < 12; ++I)
	{
		...
		FVF::TL	pv[4];
		pv[0].set(float(0), float(RenderDevice->dwHeight), c, 0, 0);
		pv[1].set(float(0), float(0), c, 0, 0);
		pv[2].set(float(RenderDevice->dwWidth), float(RenderDevice->dwHeight), c, 0, 0);
		pv[3].set(float(RenderDevice->dwWidth), float(0), c, 0, 0);
...
#if defined(USE_DX10) || defined(USE_DX11)
	//skyloader: vertical sync
	//always enabled in menu
	UINT VSync = 0;

	if (g_pGamePersistent != nullptr)
	{
		if (g_pGamePersistent->m_pMainMenu->IsActive() || psDeviceFlags.test(rsVSync))
			VSync = 1;
		else
		{
			VSync = 0;
...
void dxRenderDeviceRender::OverridePresentSurface()
{
	if (PresentRenderTarget > 0)
	{
		//RImplementation.Target->rt_Normal->
		ID3DTexture2D *pBuffer;
		R_CHK(HW.m_pSwapChain->GetBuffer(0, __uuidof(ID3DTexture2D), (LPVOID*)&pBuffer));

		//Draw over backbuffer directly

		//HW.pContext->CopyResource(pBuffer, RImplementation.Target->rt_Generic->pSurface);
		pBuffer->Release();
	}
}