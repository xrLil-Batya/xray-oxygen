...
class  XRRENDER_COMMONS_API	ConstantTableSetup;
...
struct XRRENDER_COMMONS_API	R_constant_load
{
...
struct XRRENDER_COMMONS_API	R_constant : public xr_resource
{
...
public:
	...
	virtual ~ConstantTableSetup () {}
};

class	 XRRENDER_COMMONS_API			ConstantTable	: public xr_resource_flagged	{
public:
...
public:
	...
	void					merge		(ConstantTable* C);
	ref_constant			get			(LPCSTR		name);		// slow search
	ref_constant			get			(shared_str&	name);		// fast search

	BOOL					equal		(ConstantTable& C);
	BOOL					equal		(ConstantTable* C)	{	return equal(*C);		}
	BOOL					empty		()						{	return 0==table.size();	}
private:

};
typedef	resptr_core<ConstantTable,resptr_base<ConstantTable> >				ref_ctable;
