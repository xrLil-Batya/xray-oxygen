...
//*** Bone Instance *******************************************************************************
#pragma pack(push,8)
class XRRENDER_COMMONS_API		CBoneInstance
{
public:
...
#pragma pack( push,2 )
struct XRRENDER_COMMONS_API vertBoned1W			// (3+3+3+3+2+1)*4 = 15*4 = 60 bytes
{
	Fvector	P;
	Fvector	N;
	Fvector	T;
...
struct XRRENDER_COMMONS_API vertBoned2W			// (1+3+3 + 1+3+3 + 2)*4 = 16*4 = 64 bytes
{
	u16		matrix0;
	u16		matrix1;
	Fvector	P;
...
struct XRRENDER_COMMONS_API vertBoned3W          // 70 bytes
{
	u16		m		[3];
	Fvector	P;
	Fvector	N;
...
struct XRRENDER_COMMONS_API vertBoned4W       //76 bytes
{
	u16		m		[4];
	Fvector	P;
	Fvector	N;
...
struct XRRENDER_COMMONS_API SJointLimit
{
	Fvector2		limit;
    float 			spring_factor;
...
struct XRRENDER_COMMONS_API SBoneShape
{
    enum EShapeType
	{
    	stNone,
...
struct XRRENDER_COMMONS_API SJointIKData
{
    // IK
    EJointType		type;
...
using BoneIt = xr_vector<CBone*>::iterator;

class XRRENDER_COMMONS_API CBone:
...
class 	XRRENDER_COMMONS_API	CBoneData:
	public IBoneData
{
protected:
	u16					SelfID;
