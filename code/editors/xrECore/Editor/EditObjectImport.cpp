...
bool CEditableObject::ImportLWO(const char* fn, bool optimize)
{
	...
	for (st_lwLayer* lwLayer = lwObj->layer; lwLayer; lwLayer = lwLayer->next)
	{
		...
		for (st_lwVMap* lwVmap = lwLayer->vmap; lwVmap; lwVmap = lwVmap->next)
		{
			...
			case ID_PICK: ELog.Msg(mtError, "Found 'PICK' VMAP. Import failed.");
				goto importFailed;
			case ID_MNVW: ELog.Msg(mtError, "Found 'MNVW' VMAP. Import failed.");
				goto importFailed;
			case ID_MORF: ELog.Msg(mtError, "Found 'MORF' VMAP. Import failed.");
				goto importFailed;
			case ID_SPOT: ELog.Msg(mtError, "Found 'SPOT' VMAP. Import failed.");
				goto importFailed;
			case ID_RGB: ELog.Msg(mtError, "Found 'RGB' VMAP. Import failed.");
				goto importFailed;
			case ID_RGBA: ELog.Msg(mtError, "Found 'RGBA' VMAP. Import failed.");
				goto importFailed;
			}
		}
		// points
