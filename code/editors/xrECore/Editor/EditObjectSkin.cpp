			...
			if (EPrefs->object_flags.is(epoDrawBoneShapes))
			{
				...
				if ((*b_it)->shape.Valid()) {
					...
					case SBoneShape::stCylinder:
						DU_impl.DrawCylinder(mat, (*b_it)->shape.cylinder.m_center, (*b_it)->shape.cylinder.m_direction, (*b_it)->shape.cylinder.m_height,
							(*b_it)->shape.cylinder.m_radius, c, c, TRUE, TRUE);
						break;
					}
				}
			}
...
//	Fvector axis;
	...
	for (FvectorIt I = V.begin(); I != V.end(); ++I) // To @All optimize this loop
	{
		Fvector tmp;
		Fvector pt = *I;
