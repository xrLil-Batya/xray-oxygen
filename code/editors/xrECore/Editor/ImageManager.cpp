...
bool Surface_Load(LPCSTR full_name, U32Vec& data, u32& w, u32& h, u32& a)
{
	...
	AnsiString ext = LowerCase(EFS_Utils::ExtractFileExt(full_name));
	if (ext==".tga"){
    	CImage img;
    ...
    else
    {
        ...
        if (bm){
            ...
            for (int y = h - 1; y >= 0; y--)
            {
                CopyMemory(data.data() + (h - y - 1)*w, FreeImage_GetScanLine(bm, y), w4);
            }
...
void CImageManager::CreateGameTexture(LPCSTR src_name, ETextureThumbnail* thumb)
{
	...
	FS.update_path			(game_name, _textures_,game_name); //#GIPERION: _textures_ was replaced by _textures_
...
bool CImageManager::MakeGameTexture(ETextureThumbnail* THM, LPCSTR game_name, u32* load_data)
{
	...
	// remove old
    FS.file_delete			(game_name);
    AnsiString game_name2 	= EFS.ChangeFileExt(game_name,"#.dds");
...
//.	FS.update_path			(fn,_textures_,ChangeFileExt(src_name,".tga").c_str());
	FS.update_path			(fn,_textures_,EFS_Utils::ChangeFileExt(src_name,".dds").c_str()); //#GIPERION: _textures_ was replaced by _textures_
    u32 a;
...
{   
    ...
    if (sync_game) 	FS.file_list(M_GAME,_textures_,FS_ListFiles|FS_ClampExt,"*.dds"); //#GIPERION: _textures_ was replaced by _textures_
...
//.             MakeThumbnailImage(THM,data.begin(),w,h,a);
        ...
        if (bUpdated){
            ...
            FS.update_path			(thm_fn, _textures_,    EFS.ChangeFileExt(base_name,".thm").c_str()); //#GIPERION: _textures_ was replaced by _textures_
            FS.update_path			(dds_fn, _textures_,    EFS.ChangeFileExt(base_name,".dds").c_str()); //#GIPERION: _textures_ was replaced by _textures_
            if (bForceBaseAge){
...
void CImageManager::ChangeFileAgeTo(FS_FileSet* tgt_map, int age)
{
	...
	for (; it!=_E; it++){
        ...
        FS.update_path			(dds_fn,_textures_,EFS.ChangeFileExt(base_name,".dds").c_str());
...
{                	
    return FS.file_list(files, _textures_,(bFolders?FS_ListFolders:0)|FS_ListFiles|FS_ClampExt,"*.dds"); //#GIPERION: _textures_ was replaced by _textures_
}
...
void CImageManager::RemoveTexture(LPCSTR fname, EItemType type, bool& res)
{
    ...
    }else if (TYPE_OBJECT==type)
    {
        ...
        if (FS.exist(_textures_,src_name.c_str()))
        {
            ...
            // game
            FS.file_delete		(_textures_,game_name.c_str());
            // game 2
            FS.file_delete		(_textures_,game_name2.c_str());
            return;
        }
    }
    res 						= false;
