...
u32		VCPacked::add_vert(const Fvector& V)
{
	...
	if (0xffffffff == P)
	{
		...
		if (ixE != ix)							get_element(ixE, iy, iz) = (P);
		if (iyE != iy)							get_element(ix, iyE, iz) = (P);
		if (izE != iz)							get_element(ix, iy, izE) = (P);
		if ((ixE != ix) && (iyE != iy))				get_element(ixE, iyE, iz) = (P);
		if ((ixE != ix) && (izE != iz))				get_element(ixE, iy, izE) = (P);
		if ((iyE != iy) && (izE != iz))				get_element(ix, iyE, izE) = (P);
		if ((ixE != ix) && (iyE != iy) && (izE != iz))	get_element(ixE, iyE, izE) = (P);
	}
	return P;
}
