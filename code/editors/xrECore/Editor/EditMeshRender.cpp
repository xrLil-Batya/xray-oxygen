...
void CEditableMesh::RenderSkeleton(const Fmatrix&, CSurface* S)
{
	...
	for (IntIt i_it = face_lst.begin(); i_it != face_lst.end(); ++i_it)
	{
		...
		{
			pv->P.set((pv - 1)->P);	pv->N.invert((pv - 1)->N);	pv->uv.set((pv - 1)->uv);
			++pv;
			pv->P.set((pv - 3)->P);	pv->N.invert((pv - 3)->N);	pv->uv.set((pv - 3)->uv);
			++pv;
			pv->P.set((pv - 5)->P);	pv->N.invert((pv - 5)->N);	pv->uv.set((pv - 5)->uv);
			++pv;
			++f_cnt;
		}
