...
bool EDetail::Update(LPCSTR name)
{
	...
	{
		st_Face& F = const_cast<st_Face&> (M->GetFaces()[f_id]);
		u16 ind[3];

		for (int k = 0; k < 3; ++k, ++idx)
		{
			Fvector& P = const_cast <Fvector&>(M->GetVertices()[F.pv[k].pindex]);
	...
	memcpy(indices, inds.begin()._Ptr, number_indices * sizeof(u16));
