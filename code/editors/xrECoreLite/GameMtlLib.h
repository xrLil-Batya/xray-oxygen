...
public:
	...
	IC SEGameMtl*		GetMaterialByIdx(u16 idx)		{VERIFY(idx<materials.size()); return materials[idx];}
	IC SEGameMtl*		GetMaterialByID (s32 id)		{return GetMaterialByIdx(GetMaterialIdx(id));}
#endif

	IC EGameMtlIt		FirstMaterial	(){return materials.begin();}
	IC EGameMtlIt		LastMaterial	(){return materials.end();}
...
#else
	// game
	IC SEGameMtlPair*	GetMaterialPair		(u16 idx0, u16 idx1){R_ASSERT((idx0<material_count)&&(idx1<material_count)); return material_pairs_rt[idx1*material_count+idx0];}
#endif
	IC EGameMtlPairIt	FirstMaterialPair	(){return material_pairs.begin();}
	IC EGameMtlPairIt	LastMaterialPair	(){return material_pairs.end();}
