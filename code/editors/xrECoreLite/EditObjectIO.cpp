...
bool CEditableObject::Load(const char* fname)
{
    ...
    strlwr(ext);
    //if 	(strcmp(ext, ".lwo") == 0)    		return Import_LWO(fname,false);
...
bool CEditableObject::SaveObject(const char* fname)
{
	...
	if (F){
        ...
        m_LoadName 		= fname;
        m_ObjectVersion = (int)FS.get_file_age(fname); 	VERIFY3(m_ObjectVersion>0,"Invalid file age:",fname);
        return			true;
    }else{
    	return 			false;
    }
...
//    Log("2: ",F.tell());
    ...
    for (auto sf_it=m_Surfaces.begin(); sf_it!=m_Surfaces.end(); sf_it++){
...
//    Log("4: ",F.tell());
    ...
    if (!m_SMotions.empty())
    {
        ...
        for (auto s_it=m_SMotions.begin(); s_it!=m_SMotions.end(); s_it++) (*s_it)->Save(F);
        F.close_chunk	();
    }
...
//    Log("5: ",F.tell());
    ...
    if (!m_BoneParts.empty()){
        ...
        for (auto bp_it=m_BoneParts.begin(); bp_it!=m_BoneParts.end(); bp_it++){
            ...
            for (auto s_it=bp_it->bones.begin(); s_it!=bp_it->bones.end(); s_it++)
...
bool CEditableObject::Load(IReader& F)
{
	...
	R_ASSERT2(version == EOBJ_CURRENT_VERSION, "CEditableObject: unsupported file version. Object can't load.");
	...
	if (F.find_chunk(EOBJ_CHUNK_SURFACES3)){
		...
		for (auto s_it=m_Surfaces.begin(); s_it!=m_Surfaces.end(); s_it++){
			*s_it 		= xr_new<CSurface>();
			...
			(*s_it)->SetFVF		(F.r_u32());
			cnt 				= F.r_u32();
			R_ASSERT3(cnt == 1, "Object surface '%s' has more than one TC's.", buf.c_str());
		}
	...
	if(OBJ){
		...
		for (int count=1; M; count++) {
			...
			R_ASSERT3(mesh->LoadMesh(*M), "CEditableObject: Can't load mesh '%s'!", *mesh->m_Name);
