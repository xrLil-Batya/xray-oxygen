...
public:
	...
	inline LPCSTR Name() { return name.c_str(); }
	inline int FrameStart() { return iFrameStart; }
	inline int	FrameEnd() { return iFrameEnd; }
	inline float FPS() { return fFPS; }
	inline int	Length() { return iFrameEnd - iFrameStart; }
...
public:
	struct AnimItem
	{
		...
		u16 slot;
		AnimItem() : slot(u16(-1)) {}
		inline void set(shared_str nm, u16 s) { name = nm; slot = s; }
		inline void clear() { set("", u16(-1)); }
		inline bool valid() { return !!(name.size() && (slot != u16(-1))); }
		inline bool equal(const AnimItem& d) const { return name.equal(d.name) && (slot == d.slot); }
	};
