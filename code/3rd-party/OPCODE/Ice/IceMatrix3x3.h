...
#define __ICEMATRIX3X3_H__
	...
	class ICEMATHS_API Matrix3x3
	{
		...
		//! Returns a row.
		inline_	const Point&	GetRow(const uqword r)				const	{ return *(const Point*)&m[r][0];	}
		//! Returns a row.
		inline_	Point&			GetRow(const uqword r)						{ return *(Point*)&m[r][0];			}
		//! Sets a row.
		inline_	void			SetRow(const uqword r, const Point& p)		{ m[r][0] = p.x;	m[r][1] = p.y;	m[r][2] = p.z;	}
		//! Returns a column.
		inline_	void			GetCol(const uqword c, Point& p)	const	{ p.x = m[0][c];	p.y = m[1][c];	p.z = m[2][c];	}
		//! Sets a column.
		inline_	void			SetCol(const uqword c, const Point& p)		{ m[0][c] = p.x;	m[1][c] = p.y;	m[2][c] = p.z;	}
