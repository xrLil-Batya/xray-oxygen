...
#define __ICEUTILS_H__
	...
	inline_ void	ReverseBits(uqword& n)
	{
	...
	inline_ uqword	CountBits(uqword n)
	{
	...
	//! Even faster?
	inline_ uqword	CountBits2(uqword bits)
	{
	...
	//! Order is important.
	inline_ void	SpreadBits(uqword& n)
	{
		n = ( n & 0x0000ffff) | (( n & 0xffff0000) << 16);
	...
	inline_ bool	IsPowerOfTwo(uqword n)				{ return ((n&(n-1))==0);					}
	...
	//! y ^= x;		/* y' = (y^(x^y)) = x */
	//! x ^= y;		/* x' = (x^y)^x = y */
    inline_ void	Swap(uqword& x, uqword& y) { x ^= y; y ^= x; x ^= y; }
	...
	inline_ uqword	abs_(sdword x)					{ sdword y= x >> 31;	return (x^y)-y;		}
...
//	inline_	uqword	LowestOneBit_(uqword w)			{ return ((w) & (-(w)));					}
...
//	#define PREVENT_COPY(curclass)	void operator = (const curclass& object)	{	ASSERT(!"Bad use of operator =");	}
	 ...
	 */
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	FUNCTION ICECORE_API uqword Alignment(uqword address);
	...
	inline_ void Compute2DCoords(uqword& u, uqword& v, uqword i, uqword nbu)
	{
		v = i / nbu;
		u = i - (v * nbu);
	}
	...
	inline_ void Compute3DCoords(uqword& u, uqword& v, uqword& w, uqword i, uqword nbu, uqword nbu_nbv)
	{
		w = i / (nbu_nbv);
