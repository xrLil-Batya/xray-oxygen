...
typedef struct st_SEVMap {      /* Used by SECOLTYPE_VMAPs, since both a type and a name are needed to identify a VMap */
  ...
  const char *name /* language encoded */; /* VMap Name.                                                                          */
} SEVMap;
...
typedef enum en_SEBaseTypes {
  ...
  SEBASETYPE_SCENE, /* item is name of scene (char*) (not used)*/ /* language encoded */
  SEBASETYPE_ITEM,  /* item is a LWItemID */
  SEBASETYPE_SURFACEGROUP, /* item is name of group (char*) */ /* language encoded */
...
/* choice list construction */
  ...
  const char * /* language encoded */ (*listName)    ( void *item, int index );
...
typedef struct st_SEBank
{
  ...
  char          *category /* language encoded */;                  /* name of category this bank belongs to.  can be NULL.  Banks with same category name are grouped together */
  char          *title /* language encoded */;                     /* Bank Title, for display purposes.  Used in the Bank Popup. Also used to organize column selection */
...
typedef struct st_SceneEditorGlobal
{
    ...
    int (*registerBankMaster)     ( const char *master_name /* language encoded */); /* Register a new bank master with Scene Editor.  This plug-in will be launched automatically when the scene Editor is first activated.  Returns 1 on succes, -1 if already added and 0 on error */
    int (*unregisterBankMaster)   ( const char *master_name /* language encoded */); /* Unregister a bank master.  Returns 1 on succes, -1 if already added and 0 on error                                                                                */
    unsigned int (*numInstances)  ( void);                         /* return number of instances of the Scene Editor running */
    int (*command)                ( const char *command_string /* language encoded */, void *command_return_data);   /* Issue a command to the Scene Editor global service.  Returns 0 on error */
