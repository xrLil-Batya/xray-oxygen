...
typedef struct st_LWBlockIdent {
    LWID         id;
    const char  *token /* ascii encoded */;
} LWBlockIdent;
...
#define LWFILEIOFUNCS_GLOBAL    "File IO 3"
#define LWIO_BINARY_IFF      0x8C0000
#define LWIO_BINARY_IFF32    0x8D0000

typedef struct st_LWFileIOFuncs {
    LWSaveState *       (*openSave) (const char *filename /* language encoded */, int ioMode);
    void                (*closeSave)(LWSaveState *save);
    LWLoadState *       (*openLoad) (const char *filename /* language encoded */, int ioMode);
