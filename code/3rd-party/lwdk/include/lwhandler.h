...
typedef struct st_LWInstanceFuncs {
    ...
    const char * /* language encoded */(*descln)  (LWInstance);
} LWInstanceFuncs;
...
typedef struct st_LWInterface {
    ...
    LWError     (*command) (LWInstance, const char *command /* language encoded */);
} LWInterface;
...
typedef void    LWInstUpdate (const char *className /* ascii encoded */, LWInstance);
