    ...
    int stream_size;
{
    ...
    if (version == nullptr || version[0] != my_version[0] ||
    ...
    uInt  dictLength;
{
    ...
    if (strm == nullptr || strm->state == nullptr || dictionary == nullptr ||
        strm->state->wrap == 2 ||
    ...
    z_streamp source;
{
    ...
    if (source == nullptr || dest == nullptr || source->state == nullptr) {
        return Z_STREAM_ERROR;
    }
    ...
    if (ds == nullptr) return Z_MEM_ERROR;
    ...
    if (ds->window == nullptr || ds->prev == nullptr || ds->head == nullptr ||
        ds->pending_buf == nullptr) {
        deflateEnd (dest);
        return Z_MEM_ERROR;
    }
