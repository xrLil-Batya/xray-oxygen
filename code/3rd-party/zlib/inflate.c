 ...
 * - Check next_in and next_out for nullptr on entry to inflate()
 *
...
z_streamp strm;
{
    ...
    if (strm == nullptr || strm->state == nullptr) return Z_STREAM_ERROR;
    ...
    for (;;)
        ...
        case HEAD:
            ...
            if ((state->wrap & 2) && hold == 0x8b1f) {  /* gzip header */
                state->check = crc32(0L, nullptr, 0);
            ...
            strm->adler = state->check = adler32(0L, nullptr, 0);
        ...
        case HCRC:
            ...
            strm->adler = state->check = crc32(0L, nullptr, 0);
            state->mode = TYPE;
            break;
#endif
        case DICTID:
...
z_streamp strm;
{
    ...
    if (strm == nullptr || strm->state == nullptr || strm->zfree == (free_func)0)
        return Z_STREAM_ERROR;
    state = (struct inflate_state FAR *)strm->state;
    if (state->window != nullptr) ZFREE(strm, state->window);
...
z_streamp source;
{
    ...
    /* check input */
    if (dest == nullptr || source == nullptr || source->state == nullptr ||
    ...
    if (copy == nullptr) return Z_MEM_ERROR;
    window = nullptr;
    if (state->window != nullptr) {
