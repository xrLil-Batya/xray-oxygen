...
public:
    ...
    enum SchedulingType
    {
		...
		kUntrusted,
		//----------------------------------------------------------------------
		//! OBSOLETE: These are the original enums. The default is now always
		//! serial and the others were renamed for consistency.
		kDefaultScheduling = kSerial,
		kSerialize         = kSerial,
		kGloballySerialize = kGloballySerial
    };
	...
	virtual MStatus			dependsOn( const MPlug& plug,
										const MPlug& otherPlug,
										bool& depends ) const;
