...
local tex_pos			= "$user$position"
local tex_mat			= "$user$material"
local tex_img			= "$user$generic0"
local tex_var			= "$user$generic2"
...
function normal(shader, t_base, t_second, t_detail)
	...
	shader:sampler		("s_position")	:texture(tex_pos)
	shader:sampler		("s_leaves")	:texture(tex_leaves):wrap()		:f_anisotropic()
	shader:sampler		("s_material")	:texture(tex_mat)	:clamp()	:f_linear()
	shader:sampler		("s_image")		:texture(tex_img)	:clamp()	:f_anisotropic()
end
