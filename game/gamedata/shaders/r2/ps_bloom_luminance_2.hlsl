...
float	sample	(float2	tc)	{
	float 	phalf 	= .5f/64.h;
	float4 	res;
		res.x 	= dot	(tex2D(s_image,tc+float2(-phalf,-phalf)), 1/4.h); // sum components
		res.y 	= dot	(tex2D(s_image,tc+float2(+phalf,-phalf)), 1/4.h); // sum components
		res.z 	= dot	(tex2D(s_image,tc+float2(-phalf,+phalf)), 1/4.h); // sum components
		res.w 	= dot	(tex2D(s_image,tc+float2(+phalf,+phalf)), 1/4.h); // sum components
