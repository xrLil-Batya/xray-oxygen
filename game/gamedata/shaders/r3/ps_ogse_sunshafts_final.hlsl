...
float4 main(p_screen I) : SV_Target
{
	...
	jit.x = jitter0.Sample(smp_rtlinear, float2(jtc.x, jtc.y + timers.x*0.01*SS_DUST_SPEED)*dust_size).x;
	jit.y = jitter1.Sample(smp_rtlinear, float2(jtc.x + timers.x*0.01*SS_DUST_SPEED, jtc.y)*dust_size).y;
	jit.z = s_noise.Sample(smp_rtlinear, jtc).x;
